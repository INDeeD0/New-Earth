...
function flattenLevelData(level) {
  const flat = {};

  // 1. Copy simple numeric values at the top level
  for (const [key, value] of Object.entries(level || {})) {
    if (typeof value !== "object" && value !== null) flat[key] = value;
  }

  // 2. Merge nested arrays by subtype (costs, attributes, stats, etc.)
  const mergeFields = ["limit", "storage", "attributes", "stats", "costs"];
  mergeFields.forEach(field => {
    const arr = level?.[field];
    if (Array.isArray(arr)) {
      arr.forEach(entry => {
        const name = entry?.subtype || entry?.target_subtype || "";
        if (!name) return;
        const val = entry.quantity ?? entry.value ?? 0;
        flat[name.toLowerCase()] = val;
      });
    }
  });
