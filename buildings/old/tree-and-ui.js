...
const UI = (function(Helpers, Tables, DataLoader, Totals){
    ...
    function wireTreeClicks(keyMap, objMap) {
        $('.section').on('click', function() {
            ...
            const buildingLabel = $(this).text().trim();
            const mappedKey = keyMap[buildingLabel] || buildingLabel;
            const jsonKey = mappedKey;

            // filter all tables by key
            [$('#costsTable').DataTable(), $('#rewardsTable').DataTable(), $('#statsTable').DataTable()]
            .forEach(tbl => tbl.column(0).search(jsonKey, true, false).draw());

            // hide conditional cost columns first (indices based on your structure)
            ...
            // rebuild stats table for selected building
            const structureData = window.structuresSubtypes[mappedKey.toLowerCase()];
            if (structureData && typeof Tables.rebuildStatsTableFor === "function") {
            Tables.rebuildStatsTableFor(mappedKey, structureData);
            } else {
            console.warn("No structure data found or rebuildStatsTableFor missing:", mappedKey);
            }

            // reset scale UI + redraw connectors
